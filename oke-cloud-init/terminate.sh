...
# Terminate the current OCI instance using Python SDK
terminate_instance() {
...
# Get instance OCID from metadata service
...
run_with_retry() {
    ...
    while true; do
        ...
        if [[ $elapsed -ge $max_time ]]; then
            echo "Bootstrap failed after ${max_time}s of retries: $*" >&2
            terminate_instance
        fi
        
        echo "Bootstrap failed, retrying in ${interval}s... (elapsed: ${elapsed}s/${max_time}s)" >&2
        sleep $interval
    done
}
