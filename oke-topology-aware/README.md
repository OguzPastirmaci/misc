...
> [!IMPORTANT]  
> To use the instructions in this guide, you must have a dedicated capacity pool and you must create a capacity topology. Otherwise, `rdmaTopologyData` in instance metadata service and node labels in OKE will not be available.
...
Local Block is the first latency band (Tier-0), Network Block is the second latency band (Tier-1) in OCI's RDMA networks. You can read [this blog post](https://blogs.oracle.com/cloud-infrastructure/post/first-principles-zettascale-oci-superclusters) and watch the associated YouTube for learning more about OCI's RDMA network design.
